...
**Gate: CONCERNS** → `docs/qa/gates/6.10-upgrade-downgrade.yml`

**Summary:** Upgrade/downgrade logic implemented with Stripe proration. Good UX with confirmation modals. **No tests** for tier change logic, proration calculations, or webhook sync.

**Critical Issues:**
- ❌ No integration tests for upgrade/downgrade API
- ❌ No tests for proration calculations
- ❌ No tests for scheduled downgrade logic

**Quality Score: 70/100** | **Risk: HIGH** (Billing logic untested)
