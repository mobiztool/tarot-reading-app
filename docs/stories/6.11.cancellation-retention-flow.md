...
- [x] **Task 1: Cancel Button UI** (AC: 1)
  - [ ] Add to subscription settings:
    ...
    <Card>
      <CardHeader>
        <CardTitle>ยกเลิกสมาชิก</CardTitle>
      </CardHeader>
      <CardContent>
        <p className="text-gray-600 mb-4">
          เราเสียใจที่จะเห็นคุณจากไป คุณสามารถยกเลิกได้ทุกเมื่อ
        </p>
        <Button 
          variant="outline" 
...
- [x] **Task 2: Cancellation Survey** (AC: 4)
  - [ ] Create `src/components/subscription/CancellationSurvey.tsx`:
    ...
    export function CancellationSurvey({ onComplete }: Props) {
      ...
      const REASONS = [
        { value: 'too_expensive', label: 'แพงเกินไป' },
        { value: 'not_using', label: 'ไม่ได้ใช้บ่อยพอ' },
        { value: 'missing_features', label: 'ขาดฟีเจอร์ที่ต้องการ' },
        { value: 'found_alternative', label: 'เจอทางเลือกที่ดีกว่า' },
        { value: 'technical_issues', label: 'มีปัญหาทางเทคนิค' },
        { value: 'temporary', label: 'หยุดชั่วคราว' },
        ...
        <div className="space-y-4">
          <div>
            <h3 className="font-semibold mb-3">
              ช่วยบอกเราได้ไหมว่าทำไมถึงยกเลิก?
            </h3>
            <p className="text-sm text-gray-600 mb-4">
              ข้อมูลของคุณจะช่วยให้เราปรับปรุงบริการได้ดีขึ้น
            </p>
          ...
          <div>
            ...
            <Textarea
              id="feedback"
              placeholder="บอกเราเพิ่มเติมว่าเราจะปรับปรุงอย่างไร..."
              value={feedback}
      ...
      return (
        <div className="space-y-4">
          <div className="text-center mb-6">
            <h3 className="text-2xl font-bold mb-2">รอก่อน! 😊</h3>
    ...
    function getRetentionOffers(reason: string, currentTier: SubscriptionTier): RetentionOffer[] {
      ...
      if (reason === 'too_expensive') {
        offers.push({
          ...
          description: `เพียง ฿${getDiscountedPrice(currentTier)}/เดือน ประหยัด ฿${getSavings(currentTier)}/เดือน`,
          icon: <Tag />,
          cta: 'รับส่วนลด',
      ...
      if (currentTier !== 'BASIC' && reason === 'not_using') {
        offers.push({
          id: 'downgrade',
          title: 'ลดระดับเป็น Basic',
          description: 'ลดค่าใช้จ่ายแต่ยังคงเข้าถึงฟีเจอร์หลักได้',
          icon: <ArrowDown />,
          cta: 'ลดระดับ',
      ...
      return (
        <div className="space-y-4">
          <Alert variant="warning">
            ...
            <AlertTitle>ยืนยันการยกเลิก</AlertTitle>
          </Alert>
          ...
          <RadioGroup value={cancelOption} onValueChange={setCancelOption}>
            <div className="flex items-start space-x-2 p-4 border rounded-lg">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 mt-1">
                  เข้าถึงจนถึง {formatDate(subscription.currentPeriodEnd, 'th-TH')}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-2 p-4 border rounded-lg">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 mt-1">
                  สูญเสียการเข้าถึงทันที (ไม่มีการคืนเงิน)
                </p>
              </div>
            </div>
          </RadioGroup>
    ...
    export function CancellationConfirmationEmail({ userName, tier, endDate }: Props) {
      return (
        <Html>
          <Body>
            <Container>
              <Heading>เราเสียใจที่เห็นคุณจากไป</Heading>
              
              <Text>สวัสดี {userName},</Text>
              
              <Text>
                สมาชิก {tier} ของคุณถูกยกเลิกแล้ว คุณจะยังคงเข้าถึงฟีเจอร์ทั้งหมดจนถึง {formatDate(endDate, 'th-TH')}
              </Text>
              
              <Text>หากคุณเปลี่ยนใจ คุณสามารถเปิดใช้งานสมาชิกอีกครั้งได้ทุกเมื่อ</Text>
              ...
              <Text className="text-gray-600">
                ขอบคุณที่ใช้บริการของเรา เราหวังว่าจะได้เห็นคุณอีกครั้ง ❤️
              </Text>
            </Container>
          </Body>
        </Html>
  ...
  - [ ] Show reactivation option for canceled users:
    ...
    {subscription.status === 'CANCELED' && (
      <Alert>
        ...
        <AlertTitle>ต้องการกลับมาใช้งาน?</AlertTitle>
        <AlertDescription>
          <Button onClick={handleReactivate} className="mt-2">
            เปิดใช้งานสมาชิกอีกครั้ง
          </Button>
        </AlertDescription>
      </Alert>
    )}
...
- [x] **Task 8: Analytics Dashboard** (AC: 10)
  - [ ] Track retention metrics:
    ...
    const retentionRate = (retention_offer_accepted / cancellation_attempted) × 100
...
- Created `CancellationModal.tsx` with multi-step flow (Survey → Retention → Confirmation → Success)
...
|----|-------------|--------|
| 1 | Cancel button in profile | ✅ |
| 2 | Immediate/end-of-period options | ✅ |
| 3 | Retention flow (survey + offers) | ✅ |
| 4 | Cancellation survey | ✅ |
| 5 | Retention offers (discount, pause, downgrade) | ✅ |
| 6 | Accept offer = subscription continues | ✅ |
| 7 | Cancel = access until period end | ✅ |
| 8 | Confirmation email | ✅ |
| 9 | Reactivation option | ✅ |
