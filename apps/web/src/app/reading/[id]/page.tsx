...
const POSITION_LABELS = {
  ...
  present: { th: 'ปัจจุบัน', emoji: '⏺️', color: 'from-purple-500 to-purple-600' },
...
export default function ReadingDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function fetchReading() {
      try {
        ...
        if (data.success) {
          ...
          track('reading_detail_viewed', { readingId: id });
        } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (data.success) {
        track('reading_deleted', { readingId: id });
        router.push('/history');
      } else {
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-800/50 rounded-full text-sm text-slate-400 mb-4">
              📅 {format(new Date(reading.createdAt), 'd MMMM yyyy เวลา HH:mm น.', { locale: th })}
            </div>
            <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-purple-400 via-pink-400 to-amber-400 bg-clip-text text-transparent mb-2">
              {reading.readingType === 'daily' ? '☀️ ดูดวงประจำวัน' : '🌙 ไพ่ 3 ใบ'}
            </h1>
          </div>
            ...
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6 mb-8 animate-fadeIn">
              ...
              <div className="mb-4">
                <h4 className="text-sm font-medium text-slate-400 mb-2">
                  {selectedCard.isReversed ? '🔄 ความหมายเมื่อกลับหัว' : '⬆️ ความหมาย'}
                </h4>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 max-w-md w-full">
            <h3 className="text-xl font-bold text-red-400 mb-4">⚠️ ยืนยันการลบ</h3>
